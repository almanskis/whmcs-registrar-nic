<?php
namespace Module\Registrar\Nic\Epp;

use Module\Registrar\Nic\Epp\EppProtocol;
use Module\Registrar\Nic\Epp\EppException;
    ...
    */
    ...
    public function connect(
        ...
        if (!stream_set_timeout($this->socket, $timeout)) {
            throw new EppException("Failed to set timeout on socket: {$errstr} (code {$errno})");
        }
        // Set blocking
        if (!stream_set_blocking($this->socket, 0)) {
            throw new EppException("Failed to set blocking on socket: {$errstr} (code {$errno})");
        }
    ...
    {
        return EppProtocol::getFrame($this->socket);
    }
     ...
     */
    public function sendFrame(string $xml)
    {
        return EppProtocol::sendFrame($this->socket, $xml);
    }
